...
from sklearn.model_selection import cross_val_score, RepeatedKFold, RandomizedSearchCV, KFold, train_test_split
    ...
    def run_CVs(self, **kwargs):
        ...
        if visualize:
            for i in range(n_folds):
                ...
                #ax.set_xlim(min(df[f'Obs{count}'])-10,max(df[f'Obs{count}'])+10)
                #ax.set_ylim(min(df[f'Obs{count}'])-10,max(df[f'Obs{count}'])+10)
        
                ax.set_title(f'Fold{count}')
                #r2_val = round(r2_score(df[f'Obs{count}'], df[f'Pred{count}']),2)
            ...
            r, pval = spearmanr(comb_cv_obs, comb_cv_preds)
            r2 = round(r**2,2)
            x_coord = max(comb_cv_obs)*.75
            y_coord = max(comb_cv_preds)*.99
            ax2.text(x_coord, y_coord,f'$R = {round(r,2)}$')
            ax2.set_xlabel('Observations', labelpad = 20)
            ax2.set_ylabel('Predictions', labelpad = 20)
            fig2.tight_layout()
            #fig2.savefig('crossVal_SVR.png') # Need to write code to output in the folder where the script that I'm importing 
            #fig2.savefig('crossVal_SVR.svg')
...
class LR(base):
    ...
    def run_CVs(self, **kwargs):
        df, fold_indices = super().run_CVs(model_type = LinearRegression(), 
...
class RF(base):
    ...
    def train_rf(self, **kwargs):
        #self.grid_search()
        model = RandomForestRegressor(n_estimators = self.ran_params['n_estimators'],
    ...
    def run_CVs(self, **kwargs):
        df, fold_indices = super().run_CVs(model_type = RandomForestRegressor(n_estimators = self.ran_params['n_estimators'],
                                  min_samples_leaf = self.ran_params['min_samples_leaf'],
                                  min_samples_split = self.ran_params['min_samples_split'],
                                  max_features = self.ran_params['max_features'],
                                  max_depth = self.ran_params['max_depth'],
...
class GB(base):
    ...
    def train_gb(self, **kwargs):
        self.grid_search()
        model = GradientBoostingRegressor(n_estimators=self.ran_params['n_estimators'],
                                      min_samples_leaf=self.ran_params['min_samples_leaf'],
                                      min_samples_split = self.ran_params['min_samples_split'],
                                      max_features = self.ran_params['max_features'],
                                      max_depth = self.ran_params['max_depth'])
    ...
    def run_CVs(self, **kwargs):
        df, fold_indices = super().run_CVs(model_type = GradientBoostingRegressor(n_estimators=self.ran_params['n_estimators'],
                                      min_samples_leaf=self.ran_params['min_samples_leaf'],
                                      min_samples_split = self.ran_params['min_samples_split'],
                                      max_features = self.ran_params['max_features'],
                                      max_depth = self.ran_params['max_depth']), 
...
class PLSR(base):
    ...
    def run_CVs(self, **kwargs):
        df, fold_indices = super().run_CVs(model_type = PLSRegression(n_components = self.ncomps_min_rsme),
                        features = kwargs['features'], 
                        response = kwargs['response'],
                        n_folds = kwargs['n_folds'],
                        title = kwargs['title'],
...
class SVM(base):
    ...
    def run_CVs(self, **kwargs):
        df, fold_indices = super().run_CVs(model_type = SVR(epsilon = 2),
                                           features = kwargs['features'],
                                           response = kwargs['response'],
                                           n_folds = kwargs['n_folds'],
                                           title = kwargs['title'],
